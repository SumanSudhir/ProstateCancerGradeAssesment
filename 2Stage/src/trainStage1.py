...
for split in range(nfolds):
    ...
    trainloader = DataLoader(t_dataset, batch_size=4, shuffle=True, num_workers=4)
    validloader = DataLoader(v_dataset, batch_size=4, shuffle=False, num_workers=4)

    writer = SummaryWriter(f'../OUTPUT/panda/stage1/split_{split}')
    ...
    #     l_rate = optimizer.param_groups[0]["lr"]
        ...
        if(k_score<score):
            torch.save(model.state_dict(), "../OUTPUT/panda/stage1/split_{}/efficient_b0_{}_{}.pth".format(split, epoch+1, score))
            np.savetxt(f'../OUTPUT/panda/stage1/split_{split}/valid_cm_{epoch+1}_{score}.txt', valid_cm, fmt='%10.0f')
            np.savetxt(f'../OUTPUT/panda/stage1/split_{split}/train_cm_{epoch+1}_{score}.txt', train_cm, fmt='%10.0f')
            k_score = score
